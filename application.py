...
from flask import Flask, render_template, redirect, request, url_for, copy_current_request_context,session,flash
...
def classify(features):
    ...
    w.writerow(['Prediction:']+result.tolist()+ proba_score)
    w.writerow(['--------------------------------------------------------------------------------------------------'])

    w2.writerow(['Flow #'+str(flow_count)] )
    w2.writerow(['Flow info:']+features)
    w2.writerow(['--------------------------------------------------------------------------------------------------'])
    flow_df.loc[len(flow_df)] = [flow_count]+ record + result.tolist() + proba_score + risk
...
def flow_detail():
    ...
    # Check if the flow_df DataFrame is empty or if the flow_id is invalid
    if flow_df.empty or flow_id not in flow_df['FlowID'].values:
        abort(404)  # Return a 404 error if the flow_id is not found
